...
try:
    ...
    if vv < (21,17,0): E.append('version must be 21.17.0 or newer')
except Exception: E.append('invalid semantic version')
# Config defaults stay safe
c=load('data/auth-config.json')
if c.get('enabled'):
    if not str(c.get('supabaseUrl','')).startswith('https://') or not str(c.get('publishableKey','')).startswith('sb_publishable_'): E.append('enabled auth config must use HTTPS + publishable key')
else:
    if c.get('supabaseUrl') or c.get('publishableKey'): E.append('disabled auth config should stay blank')
...
if not m or int(m.group(1))<18:E.append('offline prefix mismatch')
...
if not m or int(m.group(1))<18:E.append('SW cloud-ready version missing')
