(() => {
  ...
  async function beginGoogle(){
    ...
    try{await SMD21CloudAuth.startGoogleSignIn(returnTarget)}catch(err){consentNote.textContent=err.message;consentNote.style.color='#b92e4c';}
  }
  current.addEventListener('click',async()=>{
    account=SMD21Auth.getAccount();if(!account?.email||!requireConsent())return;
    if(account.provider==='google'&&cloudReady){const s=await SMD21CloudAuth.getValidSession();if(!s){await beginGoogle();return;}}
    proceed(account.email,account.provider||'local',{id:account.id,cloud:account.cloud===true});
  });
  q('#otherAccountBtn').addEventListener('click',async()=>{if(!requireConsent())return;if(cloudReady){await beginGoogle();return;}q('#emailModal').classList.remove('hidden');});
