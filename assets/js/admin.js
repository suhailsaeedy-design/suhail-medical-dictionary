import {getSupabase,currentSession,signOut,getMyProfile,logEvent,getLocalEvents,getSavedLocalProfile} from './auth.js?v=20.18.2';
import {getAIServiceStatus} from './ai.js?v=20.18.2';
...
async function init(){
  ...
  if(localMode){
    ...
    await Promise.allSettled([loadLocalAdmin(sess),loadAiHealth(null,true),logEvent('admin_page_open',{build:'20.18.2',mode:'local-free'})]);
  }else{
    ...
    await Promise.allSettled([loadAll(sb),logEvent('admin_page_open',{build:'20.18.2',mode:'optional-cloud'})]);
  }
}
