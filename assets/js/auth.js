...
const CONSENT_STORAGE_KEY='smd-privacy-consents-v1';
...
function consentMap(){try{return JSON.parse(localStorage.getItem(CONSENT_STORAGE_KEY)||'{}')||{};}catch{return {};}}
function consentIdentity(session){return String(session?.user?.id||session?.user?.email||'').trim().toLowerCase();}
export function hasPrivacyConsent(session=readStoredSession()){
  const id=consentIdentity(session);if(!id)return false;
  if(session?.local_only){const p=localProfile()||{};if(p.privacy_ack_at)return true;}
  const map=consentMap();return !!map[id];
}
export function rememberPrivacyConsent(session=readStoredSession(),at=new Date().toISOString()){
  const id=consentIdentity(session);if(!id)return false;const map=consentMap();map[id]=at||new Date().toISOString();localStorage.setItem(CONSENT_STORAGE_KEY,JSON.stringify(map));
  if(session?.local_only){const p=localProfile()||{};if(p.email){p.privacy_ack_at=map[id];saveLocalProfile(p);}}
  return true;
}
export function privacyConsentTimestamp(session=readStoredSession()){const id=consentIdentity(session);if(!id)return null;const p=session?.local_only?(localProfile()||{}):{};return p.privacy_ack_at||consentMap()[id]||null;}
...
export async function setOwnerReviewConsent(value){
  const sess=await currentSession();if(sess?.local_only){const p=localProfile()||{};p.allow_owner_review=false;p.privacy_ack_at=p.privacy_ack_at||new Date().toISOString();saveLocalProfile(p);rememberPrivacyConsent(sess,p.privacy_ack_at);return false;}
  const sb=await getSupabase();if(!sb)throw new Error('Cloud account is unavailable.');
  const {error}=await sb.rpc('set_owner_review_consent',{p_allow:!!value});if(error)throw error;rememberPrivacyConsent(sess);return !!value;
}
