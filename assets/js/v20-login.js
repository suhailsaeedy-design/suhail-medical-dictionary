import {CONFIG} from './config.js?v=20.18.2';
import {currentSession,signInWithGoogle,signInLocal,getSavedLocalProfile,cloudConfigured,signOut,getMyProfile,setOwnerReviewConsent,logEvent,hasPrivacyConsent,rememberPrivacyConsent} from './auth.js?v=20.18.2';
...
async function routeCurrent(){
  if(!session){const saved=getSavedLocalProfile();if(saved?.email)session=await signInLocal(saved.email);else return;}
  if(hasPrivacyConsent(session)){logEvent('login',{provider:session?.local_only?'local':'google',mode:'current'}).catch(()=>{});location.replace('./app.html');return;}
  if(!profile)profile=await loadProfile();
  if(profile?.privacy_ack_at){rememberPrivacyConsent(session,profile.privacy_ack_at);logEvent('login',{provider:session?.local_only?'local':'google',mode:'current'}).catch(()=>{});location.replace('./app.html');return;}
  const modal=bootstrap.Modal.getOrCreateInstance($('#consentModal'));$('#consentEmail').textContent=session.user?.email||'';$('#privacyAccept').checked=false;$('#consentStatus').textContent='';modal.show();
}
async function startOther(){
  if(navigator.onLine&&cloudConfigured()){
    setStatus(tx().opening);$('#otherAccountBtn').disabled=true;
    try{
      await signOut();
      ...
      setStatus(err?.message||'Could not open the Google account chooser.','error');
      $('#otherAccountBtn').disabled=false;
      return;
    }
  }
  if(CONFIG.localAccountFallback){
    const input=$('#localEmailInput');if(input)input.value='';
    const modal=bootstrap.Modal.getOrCreateInstance($('#localAccountModal'));modal.show();
    const status=$('#localAccountStatus');if(status)status.textContent='Google account selection needs internet. Offline local email is available on this device.';
    setTimeout(()=>input?.focus(),220);return;
  }
  setStatus('Google account selection is unavailable right now.','warning');
}
...
async function saveConsent(){if(!$('#privacyAccept').checked){$('#consentStatus').textContent='Please accept the Privacy Notice and Terms.';return}const btn=$('#consentContinue');btn.disabled=true;try{await setOwnerReviewConsent($('#ownerReviewOptIn').checked);rememberPrivacyConsent(session);logEvent('signup_or_consent',{provider:session?.local_only?'local':'google',privacyAccepted:true}).catch(()=>{});location.replace('./app.html')}catch(err){$('#consentStatus').textContent=err?.message||'Could not save account setup.';btn.disabled=false}}
...
async function init(){
 ...
 const hint=$('#otherAccountHint');if(hint)hint.textContent=(navigator.onLine&&cloudConfigured())?'Choose from your Google accounts':'Offline local email fallback';
 ...
 try{session=await currentSession();if(session){profile=await loadProfile();$('#currentAccountBtn').disabled=false;$('#currentEmailText').textContent=session.user?.email||'Saved account';const fromOAuth=sessionStorage.getItem('smd-oauth-inflight')==='1';sessionStorage.removeItem('smd-oauth-inflight');if(fromOAuth)await routeCurrent()}else if(savedLocal?.email){$('#currentAccountBtn').disabled=false;$('#currentEmailText').textContent=savedLocal.email}else{$('#currentAccountBtn').disabled=true}if(cloudConfigured())setStatus('Google account chooser is ready. Offline local fallback remains available.','success');else if(CONFIG.localAccountFallback)setStatus('Offline local account mode is available.','success');else setStatus('Cloud sign-in is not configured yet.','warning')}
