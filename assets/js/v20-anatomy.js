...
$('#rotateAuto').classList.toggle('active',auto);$('#rotateAuto').onclick=e=>{auto=!auto;e.currentTarget.classList.toggle('active',auto);};$('#panMode').onclick=e=>{panMode=!panMode;e.currentTarget.classList.toggle('active',panMode);};$('#zoomIn').onclick=()=>zoom=Math.min(3.2,zoom*1.18);$('#zoomOut').onclick=()=>zoom=Math.max(.38,zoom/1.18);$('#labelsToggle').onclick=e=>{document.body.classList.toggle('v20-hide-labels');e.currentTarget.classList.toggle('active',!document.body.classList.contains('v20-hide-labels'));};$('#peelLayer').onclick=peel;$('#resetView').onclick=()=>{rotX=-.06;rotY=.18;zoom=1;panX=panY=0;};$('#fullscreenAnatomy').onclick=()=>$('#anatomyStage').requestFullscreen?.();
