...
let manifest=null, currentLod='hd', sexProfile=localStorage.getItem('smd-anatomy-sex')||'male', pendingEntry=null;
let activeLayers=new Set(['skeleton']), activeRegion=null, activePreset=null, meshes=[];
let rotX=-.06,rotY=.18,zoom=1,panX=0,panY=0,drag=false,lastX=0,lastY=0,downX=0,downY=0,pointerMoved=false,panMode=false,auto=true,loadToken=0,quizTarget=null,regionFilter='all';
...
function pickFromCanvas(e){if(activeRegion||activePreset||pointerMoved||panMode)return;const r=canvas.getBoundingClientRect(),x=(e.clientX-r.left)/Math.max(1,r.width),y=(e.clientY-r.top)/Math.max(1,r.height);const key=bodyPickKey(x,y);if(key&&manifest?.regions?.[key])isolateRegion(key);}
...
function openAnatomyExplorer(layer='skeleton'){
 document.body.classList.add('v20-anatomy-explorer-open');activeLayers=new Set([layer]);activeRegion=null;activePreset=null;rotX=-.06;rotY=.18;zoom=1;panX=panY=0;if(manifest)loadActiveLayers();else pendingEntry=layer;
}
