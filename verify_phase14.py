...
# Later releases may activate Supabase with browser-safe public credentials only.
auth=load('data/auth-config.json')
if auth.get('enabled') is True:
    if not str(auth.get('supabaseUrl','')).startswith('https://'):errors.append('activated auth URL must use HTTPS')
    if not str(auth.get('publishableKey','')).startswith('sb_publishable_'):errors.append('activated auth must use publishable key')
else:
    if auth.get('supabaseUrl') or auth.get('publishableKey'):errors.append('disabled auth config must stay blank')
