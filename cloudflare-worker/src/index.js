...
export default {async fetch(request,env){
 ...
 }catch(e){return json({error:e?.message||'AI service error'},500,cors);}
}};
async function handleChat(body,user,env,cors){
 ...
 const out=await env.AI.run(MODEL,{messages:[{role:'system',content:system},...recent.slice(-14)],max_completion_tokens:900,temperature:.25,user:user.id});const answer=extractText(out);if(!answer)throw new Error('AI returned an empty response.');
...
async function handleTranslate(body,user,env,cors){
 ...
 const cached=await sb(env,`/rest/v1/term_translation_cache?term_id=eq.${encodeURIComponent(termId)}&language=eq.${encodeURIComponent(langCode)}&select=definition,explanation,reviewed&limit=1`,{method:'GET'});if(cached?.[0])return json({...cached[0],cached:true},200,cors);
 ...
 const system=`Translate the supplied medical dictionary text from English into ${language}. Preserve medical meaning, abbreviations, Latin names, drug names, and units. Do not add diagnosis or treatment advice. Return ONLY valid JSON with string keys definition and explanation. Use natural terminology suitable for medical students.`;
 const prompt=`TERM: ${term}\nDEFINITION: ${sourceDefinition}\nEXPLANATION: ${sourceExplanation}`;const out=await env.AI.run(MODEL,{messages:[{role:'system',content:system},{role:'user',content:prompt}],max_completion_tokens:900,temperature:.05,user:user.id});const raw=extractText(out);let parsed;try{parsed=JSON.parse(raw.replace(/^```json\s*/i,'').replace(/```$/,'').trim());}catch{throw new Error('AI returned an invalid translation format.');}
 const definition=String(parsed.definition||'').trim().slice(0,10000),explanation=String(parsed.explanation||'').trim().slice(0,10000);if(!definition)throw new Error('AI returned an empty translation.');
 const row={term_id:termId,language:langCode,definition,explanation,model:MODEL,reviewed:false,updated_at:new Date().toISOString()};await sb(env,'/rest/v1/term_translation_cache?on_conflict=term_id,language',{method:'POST',body:JSON.stringify(row),headers:{Prefer:'resolution=merge-duplicates,return=minimal'}});return json({definition,explanation,cached:false,reviewed:false},200,cors);
}
