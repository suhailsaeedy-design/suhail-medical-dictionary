...
if auth.get('enabled') is True:
    if not str(auth.get('supabaseUrl','')).startswith('https://') or not str(auth.get('publishableKey','')).startswith('sb_publishable_'):errors.append('enabled distributed cloud auth must use HTTPS + publishable key')
    if re.search(r'(?i)service[_-]?role|sb_secret_',str(auth.get('publishableKey',''))):errors.append('distributed cloud auth contains forbidden secret key')
elif auth.get('enabled') is False:
    if auth.get('supabaseUrl') or auth.get('publishableKey'):errors.append('disabled distributed cloud auth should remain blank')
else:errors.append('distributed cloud auth enabled flag invalid')
...
ca=adm.get('cloudAdmin',{})
if ca.get('enabled') is True and (ca.get('roleClaim')!='smd_role' or set(ca.get('allowedRoles',[]))!={'owner','admin'} or ca.get('aggregateOnly') is not True):errors.append('enabled cloud admin must remain role-gated and aggregate-only')
if adm.get('privacy',{}).get('showRawUserState') is not False or adm.get('privacy',{}).get('showOtherUserEmails') is not False:errors.append('cloud admin privacy boundary weakened')
