...
if ca.get('enabled') not in (False,True):errors.append('cloud admin enabled flag invalid')
if ca.get('enabled') is True and (ca.get('roleClaim')!='smd_role' or ca.get('aggregateOnly') is not True):errors.append('enabled cloud admin must remain role-gated and aggregate-only')
if ca.get('roleClaim')!='smd_role':errors.append('cloud admin role claim must be smd_role')
if set(ca.get('allowedRoles',[]))!={'owner','admin'}:errors.append('allowed cloud admin roles mismatch')
if ca.get('metricsRpc')!='smd_admin_metrics' or ca.get('aggregateOnly') is not True:errors.append('aggregate-only metrics contract missing')
