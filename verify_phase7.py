...
# Cloud config may remain safely disabled (Phase 7 baseline) or be explicitly activated by a later release.
if cfg.get('enabled') is True:
    if not str(cfg.get('supabaseUrl','')).startswith('https://'):errors.append('enabled cloud auth requires HTTPS Supabase URL')
    if not str(cfg.get('publishableKey','')).startswith('sb_publishable_'):errors.append('enabled cloud auth requires browser-safe publishable key')
    if cfg.get('sync',{}).get('enabled') is not True:errors.append('activated cloud auth requires cloud sync enabled')
    if cfg.get('sync',{}).get('table')!='user_app_state':errors.append('activated cloud sync must use user_app_state')
elif cfg.get('enabled') is False:
    if str(cfg.get('supabaseUrl','')).strip():errors.append('disabled supabaseUrl should be blank')
    if str(cfg.get('publishableKey','')).strip():errors.append('disabled publishableKey should be blank')
else: errors.append('cloud auth enabled flag invalid')
