# Suhail Medical Dictionary v21.18.0 — Account/PWA/Cloud Stability Checkpoint

This v21.18.0 checkpoint keeps the completed v21 feature set and adds reliable Google account switching/logout, verified full-offline download, creator profile integration, and activation against the live Supabase sync backend.
...
```
The GitHub Pages workflow runs every completed Phase verifier (Phase 1 through Phase 17), builds a clean `_site` production artifact, verifies it, and deploys only that artifact.
...
- Phase 14 keeps cloud credentials blank/disabled in the distributed checkpoint and verifies PWA version/cache coherence.
...
- Cloud Auth/Admin/AI remain optional and disabled/blank in the distributed source; the local/offline core remains functional without paid services.
